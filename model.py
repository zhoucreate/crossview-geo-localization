...
from resnet_spade import resnet50_adaibn_a, pretrained_in_weight
import timm
import numpy as np
...
def spade_norm(layer, x, mod_f):
    ...
    out1 = layer.IN(split[0].contiguous())   #将特征图沿着通道维度分割成两部分后，对第一部分的特征图进行IN操作
    out1 = out1 * (mod_f[0] + 1) + mod_f[1]  #将归一化的结果与mod_f[0]、mod_f[1]进行加权求和
    # out1 = out1 * (mod_f[0] + 1)
   # out1 = out1 * torch.exp(mod_f[0]) + mod_f[1]
    out2 = layer.BN(split[1].contiguous())   #将特征图第二部分进行BN操作
    out = torch.cat((out1, out2), 1)         #将两个归一化的结果拼接在一起
    return out
...
class Pt_ResNet50(nn.Module):
    def __init__(self, pool='avg', init_model=None, norm='spade', init_mode=['normal'], btnk=[1,0,1], conv_norm='in'):
...
class ft_net(nn.Module):
    ...
    def __init__(self, class_num, droprate=0.5, stride=2, init_model=None, pool='avg', norm='bn', adain='a'):
        ...
        self.linear = nn.Linear(2048, 1024)   #因convnext添加
...
class ft_net_convnext_lpn(nn.Module):

    def __init__(self, class_num, droprate=0.5, stride=2, init_model=None, pool='avg', block=8):
        super(ft_net_convnext_lpn, self).__init__()
        ...
        self.model.classifier = nn.Sequential()
        self.model.avgpool1d = nn.AdaptiveAvgPool1d(1024)
        self.fc = nn.Linear(1024, 2)  # 线性变换层，将2048维的特征降至2维
        self.relu_fc = nn.ReLU()

        # self.model.stages[3] = nn.Sequential()
        # self.model.norm_pre = nn.Sequential()

    def forward(self, x):
        # x = self.model(x)

        x = self.model.stem(x)
        x = self.model.stages[0](x)    #torch.Size([8, 128, 64, 64])
        x = self.model.stages[1](x)    #torch.Size([8, 256, 32, 32])
        x = self.model.stages[2](x)    #torch.Size([8, 512, 16, 16])
        x = self.model.stages[3](x)    #torch.Size([8, 1024, 15, 15]))
        x = self.model.norm_pre(x)   #torch.Size([8, 1024, 15, 15])
        x1 = self.model.head(x)
        # print(x.shape)
        # x_global = self.afim(x)
        # x_global = self.drop(self.bn(self.linear(x_global)))

        x_global = self.model.avgpool1d(x1)
        x_global = self.relu_fc(x_global)
        x_global = self.fc(x_global)

        # x_global = x_global.view(x_global.size(0), x_global.size(1))
        # split = torch.split(x, 512, 1)
        # x_global = split[0].contiguous()
        # x_global = self.model.avgpool2(x_global)
        # x_global = x_global.view(x_global.size(0), x_global.size(1))
        # x = split[1].contiguous()
        # print(self.model.stem)
        # print(self.model.stages[0])
        if self.pool == 'avg+max':
    ...
    # def spade_norm_convnext(self, x):
    #     _, channel, _, _ = x.shape
    #     half = int(0.5 * channel)
    #     split = torch.split(x, half, 1)
    #     # if mod_f[0].size()[2:] != split[0].size()[2:]:
    ...
    #     #     mod_f[1] = F.interpolate(mod_f[1], size=split[0].size()[2:], mode='nearest')
    #     if channel==128:
    #         out1 = self.model1.layer2[0].bn1.IN(split[0].contiguous())  # 将特征图沿着通道维度分割成两部分后，对第一部分的特征图进行IN操作
    #         out2 = self.model1.layer2[0].bn1.BN(split[1].contiguous())  # 将特征图第二部分进行BN操作
    #         out = torch.cat((out1, out2), 1)  # 将两个归一化的结果拼接在一起
    #         out = self.model1.layer2[0].relu(out)
    #     elif channel==256:
    #         out1 = self.model1.layer3[0].bn1.IN(split[0].contiguous())
    #         out2 = self.model1.layer3[0].bn1.BN(split[1].contiguous())
    #         out = torch.cat((out1, out2), 1)  # 将两个归一化的结果拼接在一起
    #         out = self.model1.layer3[0].relu(out)
    #     # out = torch.cat((out1, out2), 1)  # 将两个归一化的结果拼接在一起
    #     # out = self.model1.layer2[0].relu(out)
    #     return out
    ...
    def __init__(self, class_num, droprate, stride = 2, pool = 'avg', share_weight = False, VGG16=False, LPN=False, block=6, norm='bn', adain='a', circle=False, btnk=[1,0,1], conv_norm='none', Dense=False, ResNet101=False):
        ...
        if VGG16:
            ...
            if norm == 'spade':
                ...
                # self.model_2 = ft_net_LPN(class_num, stride=stride, pool=pool, block=block)
                self.model_1 = ft_net_convnext_lpn(class_num, stride=stride, pool=pool, block=block)
                self.model_2 =  ft_net_convnext_lpn(class_num, stride = stride, pool = pool, block = block)
            # self.block = self.model_1.block
        else:
            if norm == 'spade':
                self.model_1 = ft_net_spade(class_num, stride=stride, pool=pool, norm=norm, adain=adain)
                ...
                # self.model_1 =  ft_net(class_num, stride = stride, pool = pool, norm=norm, adain=adain)
                self.model_1 = ft_net_convnext(class_num, stride=stride, pool=pool) #使用convnext网络
            self.model_2 =  ft_net(class_num, stride = stride, pool = pool)
        if norm == 'spade':
        ...
        if LPN:
            ...
            if pool == 'avg+max':
                ...
                for i in range(self.block):
                    name = 'classifier'+str(i)
                    # setattr(self, name, ClassBlock(2048, class_num, droprate))
                    setattr(self, name, ClassBlock(1024, class_num, droprate))     #使用完整的ConvNext网络
                    # setattr(self, name, ClassBlock(512, class_num, droprate))  #除去ConvNext网络的第三个stages
        elif VGG16:
            self.classifier = ClassBlock(512, class_num, droprate, return_f = circle)
        elif Dense:
            ...
            self.classifier = ClassBlock(1024, class_num, droprate, return_f=circle) #使用convnext网络
            if pool =='avg+max':
                self.classifier = ClassBlock(4096, class_num, droprate)
        ...
        if self.LPN:
            if x1 is None:
                ...
                if self.norm == 'spade':
                    ...
                    x1, y1_global = self.model_1(x1)
                y1 = self.part_classifier(x1)
                # y1_global = self.classifier(x1_global)

            if x2 is None:
                y2 = None
                y2_global = None
            else:
                x2, y2_global = self.model_2(x2)
                y2 = self.part_classifier(x2)
                # y2_global = self.classifier(x2_global)
            ...
            if x3 is None:
                ...
                if self.norm == 'spade':
                    ...
                    x3, y3_global = self.model_3(x3)
                y3 = self.part_classifier(x3)
                # y3_global = self.classifier(x3_global)


            if x4 is None:
                return y1, y1_global, y2, y2_global, y3, y3_global
            else:
                x4 = self.model_2(x4)
...
# Test the model, before you train it.
    ...
    net = three_view_net(701, droprate=0.5, pool='avg', stride=1, share_weight=True, VGG16=False, LPN=True, block=8, norm='convnext', btnk=[1,0,0,0,0,0,0])
#     os.environ['CUDA_VISIBLE_DEVICES'] ='2'
#     net = three_view_net(701, droprate=0.75, pool='avg', stride=1, share_weight=True, LPN=False, norm='convnext', btnk=[1,0,0,0,0,0,0])
    # net.eval()
    ...
    input = Variable(torch.randn(8, 3, 256, 256))
    # output1,output2 = net(input,input)
    y1, y1_global, y2, y2_global, y3, y3_global = net(input,input,input)
    # output1 = net(input)
    print('net output size:')
    print(y1[0].shape)
    print(y1_global.shape)
    # print(f1.shape)
